...
async function run() {
  ...
  // Get all registered candidates in candidates collection
  const candSnap = await db.collection("candidates").get();
  const registeredEmails = new Set();
  candSnap.docs.forEach(doc => {
    const data = doc.data();
    if (data.email) {
      registeredEmails.add(data.email.toLowerCase().trim());
    }
  });

  // Get all registered guests in guestList collection
  const guestSnap = await db.collection("guestList").get();
  const guestListMap = {};
  guestSnap.docs.forEach(doc => {
    const data = doc.data();
    if (data.email) {
      guestListMap[data.email.toLowerCase().trim()] = data;
    }
  });

  console.log("=== RESULTS FOR ALL SHEET ITEMS ===");
  const notInGuestList = [];
  const notInCandidates = [];
  
  userProvidedList.forEach(item => {
    const emailKey = item.email.toLowerCase().trim();
    const guestData = guestListMap[emailKey];
    const inCandidates = registeredEmails.has(emailKey);
    
    if (!guestData) {
      notInGuestList.push(item);
    }
    if (!inCandidates) {
      notInCandidates.push({
        ...item,
        statusInGuestList: guestData ? guestData.status : "Not in guestList"
      });
    }
  });

  console.log(`\nMissing completely from guestList collection (${notInGuestList.length}):`);
  notInGuestList.forEach((item, idx) => {
    console.log(`${idx + 1}. ${item.name} (${item.email})`);
  });

  console.log(`\nDocument NOT created in candidates collection (${notInCandidates.length}):`);
  notInCandidates.forEach((item, idx) => {
    console.log(`${idx + 1}. ${item.name} (${item.email}) | Status in guestList: ${item.statusInGuestList}`);
  });
