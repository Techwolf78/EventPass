...
async function run() {
  ...
  console.log(`Registered emails count in DB (candidates): ${registeredEmails.size}`);
  ...
  console.log(`\n=== GUESTS FROM SHEET NOT REGISTERED IN APP (${notRegistered.length}) ===\n`);
  notRegistered.forEach((item, idx) => {
...
run().catch(err => {
  console.error("Error checking not entered:", err);
  process.exit(1);
});
