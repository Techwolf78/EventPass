...
async function run() {
  ...
  console.log("Fetching candidates with enrollmentType: masterclass...");
  
  // We query all candidates and filter by masterclass to be sure we match against the masterclass collection
  const snapshot = await db.collection("candidates").where("enrollmentType", "==", "masterclass").get();
  
  const dbCandidates = {};
  snapshot.docs.forEach(doc => {
    const data = doc.data();
    if (data.email) {
      dbCandidates[data.email.toLowerCase().trim()] = data.name ? data.name.trim() : "";
    }
  });

  console.log(`Found ${snapshot.size} Masterclass candidates in database.\n`);
  console.log("Comparing names and finding mismatches...\n");
  ...
  userProvidedList.forEach(userItem => {
    ...
    const dbName = dbCandidates[emailKey];
    
    if (dbName !== undefined) {
      if (dbName !== userItem.name.trim()) {
        mismatchList.push({
          email: userItem.email,
          expectedName: userItem.name,
          actualNameInDb: dbName
        });
      }
    } else {
      notRegistered.push(userItem);
    }
  });

  console.log("=== NAME MISMATCHES FOR MASTERCLASS CANDIDATES ===");
  if (mismatchList.length === 0) {
    console.log("No name mismatches found among registered Masterclass candidates!");
  } else {
    mismatchList.forEach((item, idx) => {
      console.log(`${idx + 1}. Email: ${item.email}`);
      console.log(`   Name in Sheet:   "${item.expectedName}"`);
      console.log(`   Name in DB:      "${item.actualNameInDb}"`);
    });
  }
