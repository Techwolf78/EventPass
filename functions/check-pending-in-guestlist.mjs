...
async function run() {
  ...
  // Get guestList items where status is "pending"
  const snapshot = await db.collection("guestList").where("status", "==", "pending").get();
  
  const pendingEmails = new Set();
  snapshot.docs.forEach(doc => {
    const data = doc.data();
    if (data.email) {
      pendingEmails.add(data.email.toLowerCase().trim());
    }
  });

  console.log(`Pending guest count in DB: ${pendingEmails.size}`);

  const pendingInSheet = [];
  userProvidedList.forEach(userItem => {
    const emailKey = userItem.email.toLowerCase().trim();
    if (pendingEmails.has(emailKey)) {
      pendingInSheet.push(userItem);
    }
  });

  console.log(`\n=== PENDING GUESTS FROM YOUR SHEET (${pendingInSheet.length}) ===\n`);
  pendingInSheet.forEach((item, idx) => {
...
run().catch(err => {
  console.error("Error checking pending guestlist:", err);
  process.exit(1);
});
